...
public final class EqualsHashCodeGenerator implements ILangGenerator {
	...
	public void generate(Shell parentShell, IType objectClass) {
		...
		try {
			...
			if (returnCode == Window.OK) {
				...
				boolean useGetClass = dialog.getUseGetClass();
				...
				created = generateEquals(parentShell, objectClass,
						...
						useBlocksInIfStatements, useGetClass);
	...
	private IJavaElement generateEquals(final Shell parentShell,
			...
			final boolean useBlocksInIfStatements, boolean useGetClass)
		...
		String source = createEqualsMethod(objectClass, checkedFields,
				...
				useGettersInsteadOfFields, useBlocksInIfStatements, useGetClass);
	...
	private String createEqualsMethod(final IType objectClass,
			...
			final boolean useBlocksInIfStatements, boolean useGetClass) throws JavaModelException {
	...
	private static class EqualsHashCodeDialog extends FieldDialog {
		...
		private static final String SETTINGS_USE_GETCLASS = "UseGetClass";
		...
		public EqualsHashCodeDialog(final Shell parentShell,
			...
			useGetClass= equalsSettings
					.getBoolean(SETTINGS_USE_GETCLASS);
		...
		public boolean close() {
			...
			equalsSettings.put(SETTINGS_USE_GETCLASS, useGetClass);
		...
		private Composite addCompareReferencesOption(final Composite composite) {
			...
			button.setText("Use get&Class");
			button.setLayoutData(new GridData(GridData.HORIZONTAL_ALIGN_FILL));

			button.addSelectionListener(new SelectionListener() {

				public void widgetSelected(SelectionEvent e) {
					useGetClass = (((Button) e.widget).getSelection());
				}
			...
			button.setSelection(useGetClass);
